...
base<-readRDS("res_lv_1Km_t1.rds")
  base$COD<-paste(base$NR_ZONA,base$NR_SECAO,sep = "_")
...
#######################################################################################
## Amostra restrita 2 (pares de locais de votação tradados/controles mais próximos) ###
#######################################################################################
...
reg1<-felm(QTD_VOTOS_P ~ I_esc_ini_fun_1.25Km
           ...
           + factor(NR_ANO) |COD + COD2| 0 |COD2, data = base)  
...
reg1<-felm(QTD_VOTOS_P ~ I_placebo
           ...
           + factor(NR_ANO) |COD| 0 |COD2, data = base)  
